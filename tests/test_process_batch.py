...
def test_build_config_applies_profile_and_overrides(tmp_path: Path) -> None:
    ...
    assert config.memory_pause_threshold == pytest.approx(0.92)
    assert config.memory_resume_threshold == pytest.approx(0.80)
    assert config.cpu_pause_threshold == pytest.approx(0.95)
